<x-app-layout>
    <x-slot name="header">
        <x-ui.page-header title="{{ __('Member Profile') }}" :backUrl="route('members.index')">
            <x-slot:actions>
        ...
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg mb-6">
            <div class="p-6">
                <div class="flex items-center space-x-6">
                    <div class="shrink-0">
                        <img class="h-32 w-32 object-cover rounded-full border-4 border-stone-200"
                    ...
                    <div class="flex-1">
                        ...
                        <div class="mt-3 flex items-center space-x-4">
                            <span
                                class="px-3 py-1 text-sm font-semibold rounded-full bg-sanctuary-green/10 text-sanctuary-green">
                                {{ $member->status }}
                            </span>
                            @if($member->community)
                                <a href="{{ route('communities.show', $member->community) }}"
                                    class="text-sm text-amber-600 hover:text-amber-700 hover:underline font-medium">
                        ...
                        <div class="flex flex-col space-y-2">
                            ...
                            @if($member->profile_photo_path)
                                <form method="POST" action="{{ route('members.photo.destroy', $member) }}" class="inline">
                                    @csrf
                                    @method('DELETE')
                                    <x-ui.danger-button onclick="return confirm('Are you sure you want to remove this photo?')"
                                        class="w-full">
                                        {{ __('Remove Photo') }}
                                    </x-ui.danger-button>
                                </form>
                    ...
                    <div>
                        ...
                        <dl class="space-y-4">
                            <div>
                                ...
                                <dd class="mt-1 text-lg text-slate-900">{{ $member->first_name }}
                                    {{ $member->last_name }}</dd>
                            </div>
                            <div>
                            ...
                            <div>
                                ...
                                <dd class="mt-1 text-lg text-slate-900">{{ $member->dob->format('M d, Y') }}
                                    ({{ $member->dob->age }} years old)</dd>
                            </div>
                        </dl>
                    </div>
                    <div>
                ...
                <div>
                    <dl class="space-y-4">
                        <div>
                            ...
                            <dd class="mt-1 text-lg text-slate-900">
                                {{ $member->passport_issued_at?->format('M d, Y') ?? '-' }}</dd>
                        </div>
                        <div>
                        ...
                        <div class="flex justify-between items-start">
                            <div class="flex-1">
                                ...
                                @if($record->medications)
                                    <p class="text-sm text-slate-600 mt-1"><strong>Medications:</strong> {{ $record->medications }}
                                    </p>
                                @endif
                                @if($record->notes)
                                    <p class="text-sm text-slate-600 mt-1">{{ $record->notes }}</p>
                                @endif
                                <p class="text-xs text-slate-500 mt-2">Recorded: {{ $record->recorded_at->format('M d, Y') }}
                                </p>
                            </div>
                            <div class="flex items-center space-x-2 ml-4">
                                @if($record->document_path)
                                    <a href="{{ Storage::url($record->document_path) }}" target="_blank"
                                        class="text-sanctuary-gold hover:text-amber-700">
    ...
    <x-slot name="history">
        <h3 class="text-lg font-medium text-slate-900 mb-4 px-6">{{ __('Audit Log') }}</h3>
                    ...
                    <div class="space-y-3">
                        @foreach($event->documents as $document)
                            <div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                                <div class="flex-1">
                                    <div class="font-medium text-slate-900">{{ $document->file_name }}</div>
                                    @if($document->document_type)
                                        <div class="text-sm text-slate-600">{{ $document->document_type }}</div>
                                    @endif
